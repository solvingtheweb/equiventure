<?php include("header.php"); ?>
	
	 <div id="container">
		

		<div id="top3">
		<img src="images/banner_sub.jpg" class="bannershadow">
		</div>
	
	<div id="leftnav">
			<div class="sub_menu_heading"><h1>In This Section</h1></div>
			
	
	<div id="navcontainer">
		<ul id="navlist">
		
		<li id="active"><a href="projects.php">Overview</a></li>
		
		</ul>
	</div>


	</div>



	<div id="subcontentcontainer">
	<div id="subbanner">
	<img src="images/costarica3.jpg">
	</div>

	<div id="subcontent1">
        <h1> Current Projects</h1><br>

		<p class="specialh2">Equiventure Capital is currently structuring financing across the country and Internationally.  Below is a list of some of our current projects:</p>
	
		<table class="finance">
			<tr>
				<td>Type of Project</td>
				<td>Location</td>
				<td>Amount</td>
				<td>Type of Financing</td>
			</tr>
			<tr>
				<td>Hospitality - Hotel</td>
				<td>Virginia</td>
				<td>$12M</td>
				<td>Debt/Equity</td>
			</tr>
			<tr>
				<td>Multifamily- Rentals</td>
				<td>Florida</td>
				<td>$200M</td>
				<td>Debt/Equity</td>
			</tr>
			<tr>
				<td>Resort Development</td>
				<td>Costa Rica</td>
				<td>$225M</td>
				<td>Acquisition & Construction</td>
			</tr>
			<tr>
				<td>Commercial Retail</td>
				<td>New York City</td>
				<td>$10.5M</td>
				<td>Structured Debt</td>
			</tr>
			<tr>
				<td>Healthcare Technology</td>
				<td>USA</td>
				<td>$3.5M</td>
				<td>Equity/Venture Capital</td>
			</tr>
			<tr>
				<td>Alternative Energy</td>
				<td>USA</td>
				<td>$42.0M</td>
				<td>Debt/Equity</td>
			</tr>
			<tr>
				<td>Hospital - Hotel</td>
				<td>Florida</td>
				<td>$22M</td>
				<td>Debt</td>
			</tr>
			<tr>
				<td>Multifamily</td>
				<td>New York City</td>
				<td>$26M</td>
				<td>Debt/Equity</td>
			</tr>
			<tr>
				<td>Assisted Living Complex</td>
				<td>Texas</td>
				<td>$28M</td>
				<td>Construction</td>
			</tr>
			<tr>
				<td>Commercial Retail</td>
				<td>St. Kitts</td>
				<td>$22M</td>
				<td>Construction</td>
			</tr>
			<tr>
				<td>Hotel/Resort</td>
				<td>Bahamas</td>
				<td>$20M</td>
				<td>Construction</td>
			</tr>
			<tr>
				<td>Hotel/Resort</td>
				<td>Turks &amp; Caicos</td>
				<td>$35M</td>
				<td>Construction</td>
			</tr>
			<tr>
				<td>Land Development</td>
				<td>Delaware</td>
				<td>$12M</td>
				<td>Acquisition/Develop</td>
			</tr>
			<tr>
				<td>Commercial Office</td>
				<td>Alabama</td>
				<td>$5.4M</td>
				<td>Debt Refinance</td>
			</tr>
			<tr>
				<td>Theme Park</td>
				<td>Arizona</td>
				<td>$200M</td>
				<td>Debt/Equity</td>
			</tr>
			<tr>
				<td>Entertainment Complex</td>
				<td>Pennsylvania</td>
				<td>$15M</td>
				<td>Sale/Lease Back</td>
			</tr>
		</table>	

		
	</div>
	</div>
	
	<?php include("footer.php"); ?>
	
	</div>

	<div id="belowfooter"></div>